...
class CombinedExecutor(AgentExecutor):
    def __init__(self):
        self.agent = None
        self.runner = None
    ...
    def _init_agent(self):
        if self.agent is None:
            ...
            self.runner = Runner(
                app_name="CombinedAgent",
                agent=self.agent,
                artifact_service=InMemoryArtifactService(),
                session_service=InMemorySessionService(),
                memory_service=InMemoryMemoryService(),
            )
            logger.info("CombinedExecutor initialized runner")
    ...
    async def execute(
        ...
        logger.info(f"CombinedExecutor executing query: {query}")
        ...
        try:
            ...
            logger.error(f"Error in CombinedExecutor: {e}", exc_info=True)
            await updater.update_status(
                TaskState.failed,
                message=Message(
                    message_id=str(uuid.uuid4()),
